...
**Numerals are projected into their face's plane, not stamped on top of it.** Each face's 2D plane axes are rotated and projected to screen, and that basis becomes the canvas transform the glyph is drawn through, so a numeral picks up exactly the rotation, shear and foreshortening of the polygon it sits on. Where a numeral sits (`numerals` on each face, built in `js/die-model.js`) is fixed in face-local coordinates, and its size is solved once against the face's edge planes (`resolveNumeralSize` in `js/render.js`) rather than measured from screen geometry each frame — that's what keeps numerals glued to the die as it tumbles instead of sliding and resizing. `render.js` also carries one fixed key light, used both to shade each face and to work out which way the walls of an engraved numeral face.
